...
if __name__ == '__main__':
	...
	for folder in os.listdir(directory): #1527
		...
		if folder[:2] == 'xx':
			...
			if rule == 'box':
				box_folder_count += 1
				pp(f'Ingesting to folder: {box_MediaEncryptedIdentifier}')
				...
				fund_folder_count += 1
				pp(f'Ingesting to folder: {fund_MediaEncryptedIdentifier}')
				for file in os.listdir(folder_path):
					pp(file)
					file_count += 1
					size = get_file_size(os.path.join(folder_path, file))
					...
					try:
						files = {'UploadStream': open(os.path.join(folder_path, file), 'rb')}
						upload_req = f'https://collections.newberry.org/API/UploadMedia/v3.0/UploadNewMedia?FolderRecordID={fund_MediaEncryptedIdentifier}&FileName={file}&UploadMode=ProcessFullyInBackground{token}{json_suffix}'
						r = requests.post(upload_req, files=files).json()
						resp_code = r['APIResponse']['Code']
						pp(resp_code)
						# if resp_code == 'SUCCESS':
			...
			try:
				...
				pp(f'Could not rename {folder}')
				ir_folders_that_need_renaming.append(folder)
				values = []
				value = []
				value.append(folder)
				values.append(value)
				request = sheet.values().append(spreadsheetId=spreadsheet_id, range=range_, valueInputOption='USER_ENTERED', insertDataOption='INSERT_ROWS', body={'values':values})
			...
			mostRecentLength = (time_end - time_start) / 60
			pp(f'Most recent (mins): {mostRecentLength}')
			runningTotalLength = int((runningTotalLength + mostRecentLength) / 60 / 60)
			pp(f'Total time (hours): {runningTotalLength}')
			runningAverage = runningTotalLength / folder_count
			pp(f'Average (mins): {runningAverage}')
			countRemaining = totalFiles - file_count
			timeTillComplete = int(countRemaining * runningAverage)
			str_tillComplete = timeTillComplete.strftime("%d-%m-%Y %H:%M:%S")
			pp(f'Time till completion: {str_tillComplete}')
			newTime = time.time()
